  ...
  return (
    <div className="bg-gray-200 min-h-screen py-12 print:py-0 font-sans print:bg-white text-black text-left overflow-x-hidden transition-all">
      ...
      <div className="fixed top-12 left-12 print:hidden z-[100]">
        <button 
          onClick={() => navigate(`/full-scorecard?site=${siteParam}&lang=${lang}&admin=true`)} 
          className="px-6 py-3 bg-white/90 backdrop-blur-md border border-slate-200 rounded-2xl font-black text-xs shadow-xl hover:bg-slate-900 hover:text-white transition-all flex items-center gap-3 active:scale-90 group"
        >
          <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> {t.backToAdmin}
        </button>
      </div>
    </div>
  );
